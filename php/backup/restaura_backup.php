...
// Se o servidor do usuario for local,o backup sera feito no proprio servidor de aplicacao
        ...
        if($servidor_ip == "127.0.0.1"){
            ...
            $rsync = shell_exec("sudo rsync -Crazvpt /mnt/cliente/'$diretorio_documentos' /home/$servidor_nome_compartilhamento/'$comp_nome_usuario' >>$log");
...
// Se o servidor remoto for windows, montamos o compartilhamento no mnt/servidor e executamos o rsync do /mn/cliente para o /mnt/servidor
            ...
            if($servidor_plataforma == "Windows"){
                ...
                $rsync = shell_exec("sudo rsync -Crazvpt /mnt/cliente/'$diretorio_documentos' /mnt/servidor/'$comp_nome_usuario' >>$log");
...
// Se o servidor remoto for linux,executamos apenas o rsync
                ...
                foreach ($documentos as $value) {
                    ...
                    $rsync = shell_exec("sudo rsync -Crazvpt /mnt/cliente/'$diretorio_documentos' ssh  $servidor_user_privilegio@$servidor_ip:/home/$servidor_nome_compartilhamento/'$comp_nome_usuario_novo' >>$log"); 
                }
            }
        }
...
// Se a plataforma for Linux,utilizamos o sshpass para conectar via ssh na maquina
        ...
        if($servidor_ip == "127.0.0.1"){
            ...
            $rsync = shell_exec("sudo rsync -Crazvpt $comp_usuario_adm@$ip:/'$diretorio_documentos' /home/$servidor_nome_compartilhamento/'$comp_nome_usuario' >>$log");
            ...
            if($servidor_plataforma == "Windows"){
                ...
                $mountsrv = shell_exec("sudo mount //$servidor_ip/$servidor_nome_compartilhamento /mnt/servidor -o username='$servidor_user_privilegio',password='$servidor_senha_acesso'");    
                $rsync = shell_exec("sudo rsync -Crazvpt $comp_usuario_adm@$ip:/'$diretorio_documentos' /mnt/servidor/'$comp_nome_usuario' >>$log");
                $umount = shell_exec("sudo umount /mnt/servidor");
                
            } else if ($servidor_plataforma == "Linux"){

                $connection = ssh2_connect($servidor_ip, 22); ssh2_auth_password($connection, $servidor_user_privilegio,$servidor_senha_acesso); $stream = ssh2_exec($connection, "rsync -Crazvpt $comp_usuario_adm@$ip:/'$diretorio_documentos' /home/$servidor_nome_compartilhamento/'$comp_nome_usuario' ", false); stream_set_blocking ($stream, true); 
...
$delete = mysqli_query($conexao, "DELETE FROM backups_realizados WHERE backup_id_computador = '$comp_id' ");
